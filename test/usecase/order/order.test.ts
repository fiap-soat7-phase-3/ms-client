...
import { GetVehicleController } from '@/application/controller/client/get-vehicles.controller';
import { QueueOrderController } from '@/application/controller/order/queue-order.controller';
import { UpdateVehicleUseCase } from '@/usecase/client/update-vehicle.usecase';
...
describe('OrderModule', () => {
  let orderRepository: IClientRepository;
  ...
  it('create a order', async () => {
    const controller = new CreateClientController(orderRepository, new MockProductsGateway());
    ...
    const mockOrder: ClientCreateDTO = new ClientCreateDTO();
